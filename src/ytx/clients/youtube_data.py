...
class YouTubeDataClient:
    def __init__(self, credentials: Any) -> None:
        try:
            ...
            raise ApiError(
                code="API_ERROR",
                message="google-api-python-client is required for YouTube Data access.",
            ) from exc
    ...
    def get_mine_channel(self) -> dict[str, Any]:
        ...
        if not items:
            raise ApiError(
                ...
                message="No channel found for the authenticated account.",
            )
        return items[0]
    ...
    def get_video(self, video_id: str) -> dict[str, Any]:
        ...
        if not items:
            raise ApiError(
                code="RESOURCE_NOT_FOUND", message=f"Video not found: {video_id}"
            )
        return items[0]
