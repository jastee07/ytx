...
class AnalyticsService:
    ...
    def query(self, query: AnalyticsQuery) -> AnalyticsReport:
        ...
        api_filters = resolve_filters(
            query.filters, video_id=query.video_id if query.entity == "video" else None
        )
        ...
        normalized_rows = [
            self._normalize_row(headers, row, reverse_metrics) for row in rows
        ]
        totals = (
            _compute_totals(normalized_rows, query.metrics) if normalized_rows else None
        )
        ...
        self.cache.set(
            "analytics_query", cache_key, report.model_dump(), ttl_seconds=15 * 60
        )
        return report

    def _normalize_row(
        self, headers: list[str], row: list[Any], reverse_metrics: dict[str, str]
    ) -> dict[str, Any]:
