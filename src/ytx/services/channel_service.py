...
class ChannelService:
    ...
    def get_channel(self, profile_name: str) -> ChannelProfile:
        ...
        self.cache.set("channel_profile", cache_key, channel.model_dump(), ttl_seconds=24 * 60 * 60)
        return channel

    def list_recent_videos(self, profile_name: str, limit: int, page_token: str | None = None) -> tuple[list[VideoSummary], str | None]:
        ...
        playlist = self.client.list_playlist_items(uploads_playlist_id, max_results=page_size, page_token=page_token)
        video_ids = [item["contentDetails"]["videoId"] for item in playlist.get("items", []) if item.get("contentDetails", {}).get("videoId")]
        videos = {video["id"]: video for video in self.client.get_videos(video_ids)}
        ordered = [normalize_video(videos[video_id]) for video_id in video_ids if video_id in videos]
        next_page_token = playlist.get("nextPageToken")
        self.cache.set(
            "channel_videos",
            cache_key,
            {"items": [video.model_dump() for video in ordered[:limit]], "next_page_token": next_page_token},
            ttl_seconds=15 * 60,
        )
